...
from app.shared.exceptions import EntityNotFoundException, ValidationError
...
def test_get_all_employees_success(client):
    ...
    with patch("app.controllers.employee_controller.get_employee_service") as mock_get_service:
        mock_service = AsyncMock()
        mock_get_service.return_value = mock_service
        
        mock_service.get_all_employees.return_value = [
            {"id": 1, "name": "Bob Tester", "email": "bob@test.com", "department": "QA", "date_joined": "2026-06-01"}
        ]
...
def test_create_employee_success(client):
    ...
    with patch("app.controllers.employee_controller.get_employee_service") as mock_get_service:
        ...
        mock_service.create_employee.return_value = {
            "id": 5, "name": "Alice Developer", "email": "alice@test.com", "department": "IT", "date_joined": "2026-06-02"
        }
        ...
        body = response.get_json()
        assert body["success"] is True
        assert "Employee 'Alice Developer' was successfully created with ID 5" in body["message"]
...
def test_get_employee_by_id_success(client):
    ...
    with patch("app.controllers.employee_controller.get_employee_service") as mock_get_service:
        ...
        mock_service.get_employee_by_id.return_value = {
            "id": 10, "name": "Charlie Manager", "email": "charlie@test.com", "department": "Operations", "date_joined": "2026-05-15"
        }
...
def test_update_employee_success(client):
    ...
    with patch("app.controllers.employee_controller.get_employee_service") as mock_get_service:
        ...
        mock_service.update_employee.return_value = {
            "id": 10, "name": "Charlie Updated", "email": "charlie@test.com", "department": "Executive", "date_joined": "2026-05-15"
        }
        
        payload = {"name": "Charlie Updated", "department": "Executive"}
        response = client.put("/employees/10", json=payload)
        assert response.status_code == 200
        
        body = response.get_json()
        assert body["success"] is True
        assert "Employee 'Charlie Updated' (ID 10) was successfully updated." in body["message"]
...
def test_invalid_post_payload_structure(client):
    """Test that posting completely un-parseable JSON payload causes Smorest schema validation error."""
    response = client.post("/employees/", json={"bad_field": "some data"})
    assert response.status_code == 422  # Smorest validation fails and returns unprocessable entity status
